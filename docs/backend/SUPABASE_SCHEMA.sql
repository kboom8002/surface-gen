-- Supabase schema for Wedding Surface Agent
-- Batch 6 backend lock draft.
-- This migration is intentionally explicit and should be reviewed before production use.
...
create table if not exists public.projects (
  ...
  status text not null default 'draft' check (status in ('draft','intake_ready','running','review_required','export_ready','exported','blocked')),
...
create table if not exists public.source_files (
  ...
  file_type text not null check (file_type in ('brand_factsheet','package_policy','evidence','reference','other')),
...
create table if not exists public.source_images (
  ...
  rights_status text not null default 'unknown' check (rights_status in ('approved','review_required','restricted','rejected','unknown')),
  contains_face boolean,
  contains_logo boolean,
  contains_customer_name boolean,
  consent_status text not null default 'unknown' check (consent_status in ('approved','review_required','restricted','unknown')),
...
create table if not exists public.agent_jobs (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null references public.projects(id) on delete cascade,
  job_type text not null check (job_type in ('full_factory','partial_regeneration','qa_only','export_only')),
  status text not null default 'queued' check (status in ('queued','running','paused','review_required','completed','failed','cancelled')),
  current_node text,
...
create table if not exists public.agent_job_steps (
  ...
  status text not null check (status in ('queued','running','completed','failed','skipped','waiting_for_human')),
...
create table if not exists public.generated_assets (
  ...
  slug text not null,
  summary text,
  status text not null default 'draft' check (status in ('active','draft','suspended','archived')),
  review_status text not null default 'review_required' check (review_status in ('approved','review_required','rejected','internal_only')),
  ...
  qa_status text not null default 'unchecked' check (qa_status in ('unchecked','pass','warning','fail')),
...
create table if not exists public.gallery_albums (
  ...
  review_status text not null default 'review_required' check (review_status in ('approved','review_required','rejected')),
  sort_order integer not null default 1000,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create table if not exists public.album_taxonomy_nodes (
  ...
  node_type text not null check (node_type in ('scene','subject','mood','style','composition','vibe_cluster','service_domain')),
  label text not null,
  description text,
...
create table if not exists public.visual_asset_records (
  ...
  source_type text not null check (source_type in ('actual_photo','ai_editorial_visual','ai_process_visual','ai_concept_visual','internal_moodboard','placeholder_visual')),
  file_name text,
  storage_path text,
  source_image_id uuid references public.source_images(id) on delete set null,
  prompt_id text,
...
create table if not exists public.semantic_strategy_outputs (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null references public.projects(id) on delete cascade,
  output_type text not null check (output_type in ('search_intent_map','qis_growth_registry','entity_keyword_map','local_geo_intent_map','ai_answer_opportunity_map')),
...
create table if not exists public.growth_experiments (
  ...
  status text not null default 'backlog' check (status in ('backlog','planned','running','completed','cancelled')),
  start_date date,
  review_date date,
  created_at timestamptz not null default now(),
...
create table if not exists public.human_review_items (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null references public.projects(id) on delete cascade,
  item_type text not null check (item_type in ('claim','policy','price','review','image_rights','ai_visual','final_publish','schema','other')),
  target_table text,
  target_id text not null,
  severity text not null default 'medium' check (severity in ('low','medium','high','critical')),
  reason text not null,
  suggested_action text,
  status text not null default 'open' check (status in ('open','approved','rejected','resolved','deferred')),
...
create table if not exists public.generated_json_files (
  ...
  project_id uuid not null references public.projects(id) on delete cascade,
  file_name text not null,
  file_group text not null check (file_group in ('upload','semantic_strategy','knowledge_graph','schema','ux_copy','visual_experience','growth','manifest','qa')),
  storage_path text not null,
...
create table if not exists public.export_packages (
  ...
  status text not null default 'generated' check (status in ('generated','failed','expired','revoked')),
...
-- 9. RLS enablement. Policies are defined in SUPABASE_RLS_POLICIES.sql.
