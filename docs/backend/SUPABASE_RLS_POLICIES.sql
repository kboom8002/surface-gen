-- Supabase RLS policies for Wedding Surface Agent
-- Draft policies for MVP. Review and harden before production.

-- Helper function: user can access organization
...
create policy "source_files_update_operator" on public.source_files
  for update using (public.can_operate_project(project_id)) with check (public.can_operate_project(project_id));
...
-- agent_job_steps: readable by members, writable by service role through server-side client or operator for debug only.
...
create policy "human_review_items_update_reviewer" on public.human_review_items
  for update using (public.can_review_project(project_id)) with check (public.can_review_project(project_id));
